...
@dataclass
class ViewSQLSpec:
    """Complete specification for generating a view SQL file.

    Built by ViewConfigPrompter.run(), consumed by build_view_sql().
    """

    view_name: str
    base_table: str
    view_type: str  # "aggregate", "filter", "custom"
...
class DeliverableSQLSpec:
    ...
    join_specs: list = field(default_factory=list)  # list[JoinSpec]
    order_by: str | None = None
    view_columns: dict[str, list[str]] = field(default_factory=dict)
    macro_applications: list = field(default_factory=list)  # list[MacroApplication]


def build_deliverable_sql(spec: DeliverableSQLSpec) -> str:
    """Build a complete, runnable SQL file from a DeliverableSQLSpec.

    Single report (no views): plain SELECT with no aliases.
    Multiple tables (reports + views): aliased tables with JOIN clauses.
    Macro applications inject expressions into the SELECT clause.
    """
    reports = list(spec.report_columns.keys())
    views = list(spec.view_columns.keys())

    if not reports and not views:
        return (
    ...
    macro_names = [ma.macro_name for ma in spec.macro_applications]

    header = [
        f"-- {spec.deliverable_name}.sql",
        f"-- Generated by: vp new deliverable",
        f"-- Reports: {', '.join(reports)}" if reports else None,
        f"-- Views: {', '.join(views)}" if views else None,
        f"-- Macros: {', '.join(macro_names)}" if macro_names else None,
        "",
    ]
    header = [h for h in header if h is not None]

    # Single report, no views → unaliased path
    if len(reports) == 1 and not views:
        return _build_single_report_sql(spec, header)

    return _build_multi_report_sql(spec, header)


def build_view_sql(spec: ViewSQLSpec) -> str:
    """Build a complete, runnable SQL file from a ViewSQLSpec.

    Aggregate: SELECT group_cols, AGG(col) AS col_agg FROM table GROUP BY ...
    Filter:    SELECT * FROM table WHERE <condition>
    Custom:    SELECT * FROM table (template for user to edit)
    """
    header = [
        f"-- {spec.view_name}.sql",
        f"-- Generated by: vp new view",
        f"-- Base table: {spec.base_table}",
        "",
    ]

    if spec.view_type == "aggregate":
        return _build_aggregate_view_sql(spec, header)
    elif spec.view_type == "filter":
        return _build_filter_view_sql(spec, header)
    else:
        return _build_custom_view_sql(spec, header)


def _resolve_macro_arg(
    binding_value: str,
    aliases: dict[str, str] | None,
    all_columns: dict[str, list[str]],
) -> str:
    """Resolve a macro param binding to a possibly-qualified column reference.

    binding_value formats:
      - "table_name.column_name" — explicit table qualification (used when
        the same column exists in multiple tables; wizard stores this form)
      - "column_name" — bare name, resolved to the owning table's alias
      - literal (doesn't match any column) — returned as-is (e.g. "0.5")

    aliases: {table_name: alias_letter} or None for single-table path.
    all_columns: {table_name: [col_names]} — all report + view columns.
    """
    # Explicit table.column format
    if "." in binding_value:
        table, col = binding_value.split(".", 1)
        if table in all_columns:
            if aliases:
                return f"{aliases[table]}.{col}"
            return col
        # Not a known table — treat as literal (e.g. "0.5", "3.14")
        return binding_value

    # Bare column name — find which table owns it
    if aliases:
        owning_tables = [t for t, cols in all_columns.items() if binding_value in cols]
        if len(owning_tables) == 1:
            return f"{aliases[owning_tables[0]]}.{binding_value}"
        elif len(owning_tables) > 1:
            # Ambiguous — use first table (base). The wizard should have
            # stored "table.column" for ambiguous cases, so this is a
            # defensive fallback.
            return f"{aliases[owning_tables[0]]}.{binding_value}"
    else:
        # Single table, no aliases — check if it's a column
        for cols in all_columns.values():
            if binding_value in cols:
                return binding_value

    # Not a column — literal value
    return binding_value
...
def _build_macro_expression(
    ...
    """Build 'macro_name(arg1, arg2) AS output_column' for a MacroApplication."""
    args = []
    for param_name in ma.param_bindings:
        binding = ma.param_bindings[param_name]
        args.append(_resolve_macro_arg(binding, aliases, all_columns))

    args_str = ", ".join(args)
    return f"{ma.macro_name}({args_str}) AS {ma.output_column}"


def _apply_macros_to_columns(
    col_refs: list[str],
    macro_applications: list,
    aliases: dict[str, str] | None,
    all_columns: dict[str, list[str]],
) -> list[str]:
    """Apply macro applications to a column list.

    For each macro:
    - overwrites set → replace the matching column with the macro expression
    - overwrites None → append the macro expression as a new column
    """
    result = list(col_refs)

    for ma in macro_applications:
        expr = _build_macro_expression(ma, aliases, all_columns)

        if ma.overwrites:
            overwrite_target = ma.overwrites
            replaced = False
            for i, ref in enumerate(result):
                # Match bare "col" or "alias.col"
                bare = ref.split(".")[-1] if "." in ref else ref
                if bare == overwrite_target:
                    result[i] = expr
                    replaced = True
                    break
            if not replaced:
                # Column not found in SELECT — append rather than lose it
                result.append(expr)
        else:
            result.append(expr)
...
def _build_single_report_sql(spec: DeliverableSQLSpec, header: list[str]) -> str:
    ...
    columns = list(spec.report_columns[report])

    all_columns = {**spec.report_columns, **spec.view_columns}
    columns = _apply_macros_to_columns(
        columns, spec.macro_applications, aliases=None, all_columns=all_columns,
    )
...
def _build_multi_report_sql(spec: DeliverableSQLSpec, header: list[str]) -> str:
    """SELECT with JOINs for multiple tables (reports + views)."""
    reports = list(spec.report_columns.keys())
    views = list(spec.view_columns.keys())

    # Assign aliases: reports first, then views — a, b, c, ...
    all_tables = reports + views
    aliases: dict[str, str] = {}
    for i, table in enumerate(all_tables):
        aliases[table] = chr(ord("a") + i)
    ...
    # Collect all column references: report columns then view columns
    col_refs: list[str] = []
    ...
    all_columns = {**spec.report_columns, **spec.view_columns}
    col_refs = _apply_macros_to_columns(
        col_refs,
        spec.macro_applications,
        aliases,
        all_columns,
    )
    ...
    # Add JOINs for each subsequent table
    for table in all_tables[1:]:
        alias = aliases[table]
        js = join_lookup.get(table)
        if js:
            left_alias = aliases[js.left_table]
            join_kw = f"{js.join_type} JOIN"
            lines.append(
                f"{join_kw} {table} {alias}\n"
...
def _build_aggregate_view_sql(spec: ViewSQLSpec, header: list[str]) -> str:
    ...
    for col, agg in spec.aggregate_functions.items():
    ...
    lines.append(f"FROM {spec.base_table}")
    lines.append(f"GROUP BY {', '.join(spec.group_by_columns)}")
    lines.append(";")
    return "\n".join(lines) + "\n"


def _build_filter_view_sql(spec: ViewSQLSpec, header: list[str]) -> str:
    """SELECT * with WHERE clause."""
    lines = header[:]
    lines.append("SELECT *")
    lines.append(f"FROM {spec.base_table}")
    if spec.where_clause:
        lines.append(f"WHERE {spec.where_clause}")
    else:
        lines.append("WHERE <condition>  -- edit this condition")
    lines.append(";")
    return "\n".join(lines) + "\n"


def _build_custom_view_sql(spec: ViewSQLSpec, header: list[str]) -> str:
    """Blank template for user to edit."""
    lines = header[:]
    lines.append("SELECT *")
    lines.append(f"FROM {spec.base_table}")
    lines.append(";")
